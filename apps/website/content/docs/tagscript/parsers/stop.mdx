...
`parse` throws a `StopSignal` carrying the payload. The interpreter catches it, abandons the remaining tags and returns the text rendered so far plus the message. Nothing propagates to your `await ts.run(...)` call, so you do not need a `try` block for this, and nothing is added to `response.errors` — a stop is control flow, not a failure.
