...
process. They have no console and no stack trace, so a mistake **they** can fix is worth putting in
front of them. Malformed JSON in an `{embed}`, or a parameter that is not a number. Raise a
