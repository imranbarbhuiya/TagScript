...
| module format | ESM only — no `require` condition, no `.cjs` in `dist`. Not a blocker, see §2.3 |
...
`asyncFilter` pays per node — the single-parser numbers in the `parsers` group run 600–1,500 ns.

A typical template is ~30 Effect operations, so expect **+3 µs, around +50% on the typical case**.

There is an offsetting win. **No parser in either package is actually async** — zero `async`/`await`
in `packages/tagscript/src` outside `Interpreter` itself. Yet every node pays for `asyncFilter`'s
`Promise.all` across every registered parser plus an `await` per `parse`. A sync-first Effect design
that only suspends when a parser genuinely suspends can plausibly beat the current numbers. The
benchmark is now in the repo, so this is measurable rather than arguable.
...
- It only breaks if the ESM graph uses top-level await. `effect`'s `dist` has none — checked.

So the CJS build keeps working. The README should state a Node floor of `^20.19.0 || >=22.12.0` for
the Effect surface, and note that older Node needs either an upgrade or a move to ESM. Worth fixing
...
- Every third-party parser needs rewriting for the Effect surface. Today a parser is ~10 lines of
...
| `StopSignal`            | the render so far, then its message | nothing — control flow, not a failure  |
...
The Effect surface should keep exactly this split, as
...
## 4. Recommended approach: dual surface, not a rewrite
...
- Both surfaces are thin wrappers over one engine — the split Phase 0 already made.

CJS consumers keep the classic entry either way, and per §2.3 `require(esm)` means even they could
reach the Effect surface on a current Node.

### 4.1 Package layout

```
packages/tagscript/
  src/index.ts          -> classic surface, no effect import  (esm + cjs)
  src/effect/index.ts   -> Effect surface                     (esm only)
...
Phase 0 already landed on the classic surface.
...
**On the Effect surface** — the parser can now actually enforce the cooldown:
...
4. **Pure engine extracted** to `src/lib/Interpreter/engine.ts`: `buildNodeTree`, `checkWorkload`,
   `textDeform`, `translateNodes`. This is the shared core both surfaces will sit on, and it is what
...
### Phase 1 — the Effect surface

9. `effect` as an optional peer dep; second tsdown entry, `external: ['effect', /^effect\//]`.
10. Error hierarchy mirroring Phase 0's, with `Data.TaggedError`.
11. `Parser<E, R>` and the Effect `Interpreter`, over the Phase 0 engine.
12. `fromClassic` / `toClassic` adapters, the latter constrained to `R = never`.
13. Port the built-in parsers. `Random` and `FiftyFifty` take `effect/Random`.
14. `Context.Reference` for `TagLimit`, `CharLimit`, `ParenType` — Phase 0's `RunOptions` maps onto
    these one-to-one, which was the point of doing it first.
15. Lint rule preferring deep `effect/*` imports over the barrel, per §2.1's revised reasoning.
16. Drop the IIFE build and point `unpkg` at the ESM entry (§2.4).
17. Fix `engines.node` to `^20.19.0 || >=22.12.0` (§2.3).

### Phase 2 — the Discord plugin

18. `@tagscript/plugin-discord@5`, same dual-surface layout.
...
1. **Dual surface — decided, go.** Keep the classic entry dependency-free; add `tagscript/effect`
   with `effect` as an optional peer.
2. **Ship on RC or wait for GA?** Publishing the Effect surface under a `next` tag now gets feedback
