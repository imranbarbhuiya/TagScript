...
describe('IntersectionStatementParser', () => {
	...
	test('GIVEN a string in intersection parser THEN returns the string if all conditions are true else return empty string', async () => {
		...
		expect(rendered(await ts.run(text1))).toStrictEqual(
			rendered(new Response().setValues('You must provide a number between 100 and 1000.', text1)),
		);

		const text2 = '{all(282>=100|282<=1000):You picked 282.|You must provide a number between 100 and 1000.}';
		expect(rendered(await ts.run(text2))).toStrictEqual(rendered(new Response().setValues('You picked 282.', text2)));
	});
});
...
describe('UnionStatementParser', () => {
	...
	test('GIVEN a string in union parser THEN returns the string if any conditions are true else return empty string', async () => {
		...
		expect(rendered(await ts.run(text1))).toStrictEqual(rendered(new Response().setValues('Hello!', text1)));

		const text2 = "{any(what's up==hi|what's up==hello|what's up==hey):Hello!|How rude.}";
		expect(rendered(await ts.run(text2))).toStrictEqual(rendered(new Response().setValues('How rude.', text2)));
	});
});
...
describe('IfStatementParser', () => {
	...
	test('GIVEN a string in union parser THEN returns the string if any conditions are true else return empty string', async () => {
		...
		expect(rendered(await ts.run(text1))).toStrictEqual(
			rendered(new Response().setValues('You guessed it! The number I was thinking of was 63!', text1)),
		);

		const text2 =
			'{if(73==63):You guessed it! The number I was thinking of was 63!|Too {if(73<63):low|high}, try again.}';
		expect(rendered(await ts.run(text2))).toStrictEqual(
			rendered(new Response().setValues('Too high, try again.', text2)),
		);

		const text3 =
			'{if(14==63):You guessed it! The number I was thinking of was 63!|Too {if(14<63):low|high}, try again.}';
		expect(rendered(await ts.run(text3))).toStrictEqual(
			rendered(new Response().setValues('Too low, try again.', text3)),
		);

		const text4 = '{=(a):{if(52==63):You guessed it! The number I was thinking of was 63!}}{a}';
		expect(rendered(await ts.run(text4))).toStrictEqual(
			rendered(
				new Response({
		...
		expect(rendered(await ts.run(text5))).toStrictEqual(
			rendered(
				new Response({
