...
describe('SafeObjectTransformer', () => {
	...
	test('GIVEN an object with nested key THEN return the value', async () => {
		...
		expect(
			rendered(
				await ts.run('{obj.name.first.second}', {
		...
		expect(
			rendered(
				await ts.run('{obj.name.first.second.third}', {
